...
namespace FunctionalTests.Facts.Security.Facebook
{
    public class FacebookAuthentication
    {
        ...
        public async Task Security_FacebookAuthentication(HostType hostType)
        {
            using (ApplicationDeployer deployer = new ApplicationDeployer())
            {
                ...
                Assert.Equal("https://www.facebook.com/v2.8/dialog/oauth", response.Headers.Location.AbsoluteUri.Replace(response.Headers.Location.Query, string.Empty));
                var queryItems = response.Headers.Location.ParseQueryString();
                Assert.Equal("code", queryItems["response_type"]);
                Assert.Equal("550624398330273", queryItems["client_id"]);
                Assert.Equal(applicationUrl + "signin-facebook", queryItems["redirect_uri"]);
                ...
                Assert.Equal("Facebook", response.Content.ReadAsStringAsync().Result);
                ...
                Assert.Equal<HttpStatusCode>(HttpStatusCode.InternalServerError, response.StatusCode);
                Assert.Equal("SignIn_Failed", response.Content.ReadAsStringAsync().Result);
        ...
        internal void FacebookAuthenticationConfiguration(IAppBuilder app)
        {
                    ...
                    OnAuthenticated = async context =>
                    {
                        await Task.Run(() =>
                        {
                            if (context.Identity != null)
                            {
                                ...
                                Assert.Equal("https://www.facebook.com/myLink", context.Link);
                                Assert.Equal("Owinauthtester Owinauthtester", context.Name);
                                Assert.Equal("owinauthtester.owinauthtester.7", context.UserName);
    ...
    public class FacebookChannelHttpHandler : WebRequestHandler
    {
        protected async override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if (request.RequestUri.AbsoluteUri.StartsWith("https://graph.facebook.com/v2.8/oauth/access_token"))
            {
                if (queryParameters["grant_type"] == "authorization_code")
                {
                    if (queryParameters["code"] == "ValidCode")
                    {
                        ...
                        Assert.Equal("550624398330273", queryParameters["client_id"]);
                        Assert.Equal("10e56a291d6b618da61b1e0dae3a8954", queryParameters["client_secret"]);
            ...
            {
                Assert.NotNull(queryParameters["appsecret_proof"]);
